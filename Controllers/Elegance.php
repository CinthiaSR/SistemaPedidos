...
class Elegance extends Controllers{
...
// ---------------------------------------------------------------------------Muestra la pantalla principal de pedidos
	public function Elegance(){
		...
		$data['page_tag'] = "Neolux Motorizadas";
		$data['page_title'] = "Neolux Motorizadas <small>Pedidos</small>";
		$data['page_name'] = "elegance";
		$data['page_functions_js'] = "functions_elegance.js";
		$this->views->getView($this,"elegance",$data);
	}	
// -------------------------------------------------------------------ACCIONES PARA PEDIDO
	...
	public function getPedidos(){
			if($_SESSION['permisosMod']['r']){
				...
				for ($i=0; $i < count($arrData); $i++) {
					...
					if($_SESSION['permisosMod']['r']){
						$btnView .= ' <a title="Ver Detalle" href="'.base_url().'/elegance/orden/'.$arrData[$i]['idpedido'].'" class="btn btn-info btn-sm"> <i class="far fa-eye"></i> </a>';
					}
					if($_SESSION['permisosMod']['u']){
	...
	public function orden($idpedido){
		...
		$data['page_tag'] = "Pedido Neolux Motorizadas ";
		$data['page_title'] = "Pedido Neolux Motorizadas";
		$data['page_name'] = "pedido";
		$data['page_functions_js'] = "functions_elegances.js";
			...
			if( empty($_POST['idPedidoN']))
			   {
				...
				$strTypesa = ucwords(strClean($_POST['Pesa_neolux']));
			...
			if($_SESSION['permisosMod']['r']){
				if(is_numeric($idpedido)){
					...
					if(empty($data)){
						...
						ob_end_clean();
						$html = getFile("Template/Modals/PDFClientes/PDFClienteNMot",$data);
		...
		if($_SESSION['permisosMod']['r']){
			if(is_numeric($idpedido)){
				...
				if(empty($data)){
					...
					$idpedido = $data['orden']['cn'];
					ob_end_clean();
					$html = getFile("Template/Modals/PDFProduccion/PDFProduccionNMot",$data);
					$html2pdf = new Html2Pdf('L','A4','es','true','UTF-8');
		...
		if($_SESSION['permisosMod']['r']){
			if(is_numeric($idpedido)){
				...
				if(empty($data)){
					...
					$idpedido = $data['orden']['cn'];
					ob_end_clean();
					$html = getFile("Template/Modals/PDFShutters/pdfNmot",$data);
