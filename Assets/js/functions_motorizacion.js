...
tablePedidos = $('#tablePedidos').dataTable( {
    ...
    "ajax":{
        "url": " "+base_url+"/Motorizacion/getPedidos",
        "dataSrc":""
    },
    "columns":[
        {"data":"cn"},
    ...
    if(document.querySelector("#formPedidoGral")){
        let formPedidoGral = document.querySelector("#formPedidoGral");
        formPedidoGral.onsubmit = function(e) {
            e.preventDefault();
            ...
            let ajaxUrl = base_url+'/Motorizacion/setPedido'; 
            let formData = new FormData(formPedidoGral);
            ...
            request.onreadystatechange = function(){
                if(request.readyState == 4 && request.status == 200){
                    ...
                    if(objData.status){
                        ...
                        formPedidoGral.reset();
                        swal("Pedido Roller Motorizada", objData.msg ,"success");
...
function openModal()
{
    ...
    document.querySelector("#formPedidoGral").reset();
...
function fntEditPedido(idpedido){
    ...
    var ajaxUrl = base_url+'/Motorizacion/getPedido/'+idpedido;
    request.open("GET",ajaxUrl,true);
    request.send();
    request.onreadystatechange = function(){
        ...
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status)
            {
               ...
               document.querySelector("#tipoPersiana").value = objData.data.tipoPersiana;
               document.querySelector("#Motor").value = objData.data.tipoMotor;
...
function fntDelPedido(idpedido){
    ...
    swal({
        ...
        {
            var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
            var ajaxUrl = base_url+'/Motorizacion/delPedido/';
