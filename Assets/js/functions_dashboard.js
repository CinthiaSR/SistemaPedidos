...
function fntSearchVAnio1(){
    let anio = document.querySelector(".Anio1").value;
    if(anio == ""){
...
function fntSearchVAnio2(){
    let anio = document.querySelector(".ventas2").value;
    if(anio == ""){
        swal("", "Ingrese año " , "error");
        return false;
    }else{
        let request = (window.XMLHttpRequest) ? 
            new XMLHttpRequest() : 
            new ActiveXObject('Microsoft.XMLHTTP');
        let ajaxUrl = base_url+'/Dashboard/ventasAnio2';
        divLoading.style.display = "flex";
        let formData = new FormData();
        formData.append('anio',anio);
        request.open("POST",ajaxUrl,true);
        ...
        request.onreadystatechange = function(){
            if(request.readyState != 4) return;
            if(request.status == 200){
                $("#grafica2").html(request.responseText);
                divLoading.style.display = "none";
                return false;
            }
        }
...
function fntSearchVAnioSh(){
    let anio = document.querySelector(".VShuttersAnio").value;
    if(anio == ""){
        swal("", "Ingrese año " , "error");
        return false;
    }else{
        let request = (window.XMLHttpRequest) ? 
            new XMLHttpRequest() : 
            new ActiveXObject('Microsoft.XMLHTTP');
        let ajaxUrl = base_url+'/Dashboard/ventasShutters';
        divLoading.style.display = "flex";
        let formData = new FormData();
        formData.append('anio',anio);
        request.open("POST",ajaxUrl,true);
        ...
        request.onreadystatechange = function(){
            if(request.readyState != 4) return;
            if(request.status == 200){
                $("#VentasShutters").html(request.responseText);
                divLoading.style.display = "none";
                return false;
            }
        }
