...
tablePedidos = $('#tablePedidos').dataTable( {
    ...
    "ajax":{
        "url": " "+base_url+"/Balance/getPedidos",
        "dataSrc":""
    },
    "columns":[
        {"data":"cn"},
    ...
    if(document.querySelector("#formPedidoBalance")){
        let formPedidoRoller = document.querySelector("#formPedidoBalance");
        formPedidoRoller.onsubmit = function(e) {
            e.preventDefault();
            let strCN = document.querySelector('#cn').value;
            let strsucursal = document.querySelector('#suc').value;
            let strvendedor = document.querySelector('#vend').value;
            let strmedido = document.querySelector('#medid').value;
            let strentrega = document.querySelector('#entre').value;
            let strdate = document.querySelector('#date').value;
            ...
            let ajaxUrl = base_url+'/Balance/setPedido'; 
            ...
            request.onreadystatechange = function(){
                if(request.readyState == 4 && request.status == 200){
                    ...
                    if(objData.status){
                        ...
                        swal("Pedido Balance", objData.msg ,"success");
...
function openModal()
{
    ...
    document.querySelector("#formPedidoBalance").reset();
...
function fntViewInfo(idpedido){
    ...
    let ajaxUrl = base_url+'/Balance/getPedido/'+idpedido;
    request.open("GET",ajaxUrl,true);
    request.send();
    request.onreadystatechange = function(){
        if(request.readyState == 4 && request.status == 200){
...
function fntEditPedido(idpedido){
    ...
    var ajaxUrl = base_url+'/Balance/getPedido/'+idpedido;
    request.open("GET",ajaxUrl,true);
    request.send();
    request.onreadystatechange = function(){
        ...
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status)
            {
               ...
               document.querySelector("#cn").value = objData.data.cn;
               document.querySelector("#suc").value = objData.data.sucursal;
               document.querySelector("#vend").value = objData.data.vendedor;
               document.querySelector("#medid").value = objData.data.medido;
               document.querySelector("#entre").value = objData.data.entrega;
               document.querySelector("#date").value = objData.data.fecha;
...
function fntDelPedido(idpedido){
    ...
    swal({
        ...
        {
            var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
            var ajaxUrl = base_url+'/Balance/delPedido/';
