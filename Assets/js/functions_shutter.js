...
tablePedidos = $('#tablePedidos').dataTable( {
    ...
    "ajax":{
        "url": " "+base_url+"/Shutters/getPedidos",
        "dataSrc":""
    },
    "columns":[
        // {"data":"idpedido"},
    ...
    if(document.querySelector("#formPedidoShutters")){
        let formPedidoRoller = document.querySelector("#formPedidoShutters");
        formPedidoRoller.onsubmit = function(e) {
            ...
            if(strCN == '' || strsucursal == '' || strvendedor == '' || strmedido == '' || strentrega== '' || ft2== '' )
            {
            ...
            let ajaxUrl = base_url+'/Shutters/setPedido'; 
            ...
            request.onreadystatechange = function(){
                if(request.readyState == 4 && request.status == 200){
                    ...
                    if(objData.status){
                        $('#modalFormPedidoShutters').modal("hide");
                        formPedidoRoller.reset();
                        swal("Pedido Shutters", objData.msg ,"success");
...
function openModal()
{
    ...
    document.querySelector("#formPedidoShutters").reset();
    $('#modalFormPedidoShutters').modal('show')
}
// ***************************************************SELECT CLIENTE
function fntCliente(){
...
function fntEditPedido(idpedido){
    ...
    var ajaxUrl = base_url+'/Shutters/getPedido/'+idpedido;
    request.open("GET",ajaxUrl,true);
    request.send();
    request.onreadystatechange = function(){
...
function fntDelPedido(idpedido){
    ...
    swal({
        ...
        {
            var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
            var ajaxUrl = base_url+'/Shutters/delPedido/';
