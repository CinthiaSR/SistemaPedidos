...
tablePedidos = $('#tablePedidos').dataTable( {
    ...
    "ajax":{
        "url": " "+base_url+"/Neolux/getPedidos",
        "dataSrc":""
    },
    "columns":[
        // {"data":"idpedido"},
    ...
    if(document.querySelector("#formPedidoNeolux")){
        ...
        formPedidoNeolux.onsubmit = function(e) {
            ...
            let ajaxUrl = base_url+'/Neolux/setPedido'; 
...
function fntViewInfo(idpedido){
    ...
    let ajaxUrl = base_url+'/Neolux/getPedido/'+idpedido;
    request.open("GET",ajaxUrl,true);
    request.send();
    request.onreadystatechange = function(){
        if(request.readyState == 4 && request.status == 200){
...
function fntEditPedido(idpedido){
    ...
    var ajaxUrl = base_url+'/Neolux/getPedido/'+idpedido;
    request.open("GET",ajaxUrl,true);
    request.send();
    request.onreadystatechange = function(){
...
function fntDelPedido(idpedido){
    ...
    swal({
        ...
        {
            var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
            var ajaxUrl = base_url+'/Neolux/delPedido/';
