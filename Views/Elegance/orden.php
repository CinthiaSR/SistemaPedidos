        ...
        <br>
                    ...
                    $importe = 0;
                        if(count($roller) > 0){
                            foreach ($roller as $persiana) {
                                $importe +=$persiana['ne_precio'];
                     ?>
                  <tr>
                    <td><?= $persiana['ne_cantidad'] ?></td>
                    <!-- <td><= $persiana['en_identificacion'] ?></td> -->
                    <?php
                    if(empty($persiana['ne_nota'])){ ?>
                    <td><span class="badge badge-success">Normal</span></td>
                    <?php }else{ ?>
                      <td><span class="badge badge-warning">Especial</span></td>
                    <?php }?>
                    <!-- <td><= $persiana['sheer_typecontrol'] ?></td> -->
                    <td><?= $persiana['ne_ancho'] ?></td>
                    <td><?= $persiana['ne_largo'] ?></td>
                    <td><?= $persiana['ne_color_tela'] ?></td>
                    <td><?= $persiana['ne_precio'] ?></td>
                    <td>
                          <!-- <button class="btn btn-info btn-sm"    onClick="fntViewNeolux(<?=$persiana['ID_MOTORIZADA']?>)" title="Ver persiana"><i class="far fa-eye"></i> </button> -->
                          <button class="btn btn-primary btn-sm" onClick="fntEditNeolux(<?=$persiana['ID_MOTORIZADA']?>)" title="Editar persiana"><i class="fas fa-pencil-alt"></i></button>
                          <button class="btn btn-danger btn-sm"   onClick="fntDelNeolux(<?=$persiana['ID_MOTORIZADA']?>)" title="Eliminar persiana"><i class="far fa-trash-alt"></i></button>                                 
                    </td>
                  </tr>
                  <?php 
                            }
          ...
          <form id="formPrice" name="formPrice" class="form-horizontal">
              ...
              <input type="hidden" id="id" name="id" value="<?= $precio['id']?>">
                <tfoot>
                    ...
                    <th colspan="4" class="text-right">Anticipo:</th> 
                        <td class="text-right"><input type="text"  class="form-control" onkeyup="calculateTotal();" id="txtAnticipo" name="txtAnticipo"  value="<?= $precio['anticipo'] ?>" ></td>
                    </tr>
                    <tr>
            ...
            <div class="row d-print-none mt-2">
             ...
             <br><br>
             <?php
                    if(isset($roller['ne_nota'])){ ?>
                      <p>No hay notas especiales</p>
                    <?php }else{ ?>
                      <a class="btn btn-outline-info" href="<?=base_url().'/elegance/PDFNOTES/'.$orden['idpedido']?>" target="_blanck"><i class="fa fa-print"></i> Imprimir notas especiales</a> 
                    <?php }?>
             
              <a class="btn btn-success" href="<?=base_url().'/elegance/facturacliente/'.$orden['idpedido']?>" target="_blanck"><i class="fa fa-print"></i> PDF Cliente</a> 
               <a class="btn btn-warning" href="<?=base_url().'/elegance/facturaproduccion/'.$orden['idpedido']?>" target="_blanck" ><i class="fa fa-print"></i> PDF Produccion</a>
              </div> 
          </div>
          </form>
        </section>
