<?php 
    headerAdmin($data); 
    getModal('ModalBalance',$data);
    // getForm('balances',$data);
?>
    ...
    <div class="col-md-12">
      <div class="tile">
        ...
        <?php }else{
            ...
            $balance = $data['arrPedido']['balance'];
        ...
        <section id="sPedido" class="invoice">
          ...
          <!-- --------------------------------------------ADD -->
          <div>
            <h2> Balance de madera
            <?php if($_SESSION['permisosMod']['w']){ ?>
                <button class="btn btn-primary" type="button" onclick="openModalbalance();"><i class="fas fa-plus-circle"></i> Nuevo</button>
              <?php } ?>
            </h2>
        </div>
        <br>
          ...
          <div class="row">
            ...
            <table class="table table-hover table-bordered" id="tableBlindsbalance">
                      <thead>
                        <tr>
                          <th>Cant</th>
                          <th>Tipo</th>
                    ...
                    $importe = 0;
                        if(count($balance) > 0){
                            foreach ($balance as $persiana) {
                                $importe +=$persiana['bal_preciototal'];
                     ?>
                  <tr>
                    <td><?= $persiana['bal_cantidad'] ?></td>
                    <!-- <td><= $persiana['en_identificacion'] ?></td> -->
                    <?php
                    if(empty($persiana['bal_nota'])){ ?>
                    <td><span class="badge badge-success">Normal</span></td>
                    <?php }else{ ?>
                      <td><span class="badge badge-warning">Especial</span></td>
                    <?php }?>

                    <td><?= $persiana['bal_media'] ?></td>
                    <td><?= $persiana['bal_alto'] ?></td>
                    <td><?= $persiana['bal_largo'] ?></td>
                    <td><?= $persiana['bal_color'] ?></td>
                    <td>$ <?= $persiana['bal_preciototal'] ?></td>
                    <td>
                          <!-- <button class="btn btn-info btn-sm"    onClick="fntViewbalance(<?=$persiana['ID_BALANCE']?>)" title="Ver balance"><i class="far fa-eye"></i> </button> -->
                          <button class="btn btn-primary btn-sm" onClick="fntEditbalance(<?=$persiana['ID_BALANCE']?>)" title="Editar balance"><i class="fas fa-pencil-alt"></i></button>
                          <button class="btn btn-danger btn-sm"   onClick="fntDelbalance(<?=$persiana['ID_BALANCE']?>)" title="Eliminar balance"><i class="far fa-trash-alt"></i></button>                                 
                    </td>
                  </tr>
                  <?php 
                            }
          ...
          <form id="formPrice" name="formPrice" class="form-horizontal">
            ...
            <div class="row d-print-none mt-2">
             ...
             <br><br>
             <?php
                    if(isset($balance['bal_nota'])){ ?>
                    <span class="badge badge-info" style="font-size: 16px;">Sin notas especiales</span>
                    <?php }else{ ?>
                      <a class="btn btn-outline-info" href="<?=base_url().'/balance/PDFNOTES/'.$orden['idpedido']?>" target="_blanck"><i class="fa fa-print"></i> Imprimir notas especiales</a> 
                    <?php } ?>
             
              <a class="btn btn-success" href="<?=base_url().'/balance/facturacliente/'.$orden['idpedido']?>" target="_blanck"><i class="fa fa-print"></i> PDF Cliente</a> 
               <a class="btn btn-warning" href="<?=base_url().'/balance/facturaproduccion/'.$orden['idpedido']?>" target="_blanck" ><i class="fa fa-print"></i> PDF Produccion</a>
              </div> 
          </div>
          </form>
        </section>
