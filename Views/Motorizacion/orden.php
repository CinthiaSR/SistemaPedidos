    ...
    <div class="col-md-12">
      <div class="tile">
        ...
        <section id="sPedido" class="invoice">
          ...
          <!-- --------------------------------------------ADD -->
          <div>
            <h2> Roller Motorizadas
            <?php if($_SESSION['permisosMod']['w']){ ?>
                <button class="btn btn-primary" type="button" onclick="openModalRoller();"><i class="fas fa-plus-circle"></i> Agregar persiana</button>
                <!-- <button class="btn btn-info" type="button" onclick="openModalComp();"><i class="fas fa-plus-circle"></i> Agregar complemento</button> -->
        ...
        <br>
          ...
          <div class="row">
            ...
            <table class="table table-hover table-bordered" id="tableBlindsRollers">
                      <thead>
                        <tr>
                          <th>Cant</th>
                          <th>Tipo</th>
                    ...
                    $importe = 0;
                        if(count($roller) > 0){
                            foreach ($roller as $persiana) {
                                $importe +=$persiana['mo_precio'];
                     ?>
                  <tr>
                    <td><?= $persiana['mo_cantidad'] ?></td>
                    <!-- <td><= $persiana['en_identificacion'] ?></td> -->
                    <?php
                    if(empty($persiana['mo_nota'])){ ?>
                    <td><span class="badge badge-success">Normal</span></td>
                    <?php }else{ ?>
                      <td><span class="badge badge-warning">Especial</span></td>
                    <?php }?>

                    <td><?= $persiana['mo_ancho'] ?></td>
                    <td><?= $persiana['mo_largo'] ?></td>
                    <td><?= $persiana['mo_color_tela'] ?></td>
                    <td><?= $persiana['mo_precio'] ?></td>
                    <td>
                          <!-- <button class="btn btn-info btn-sm"    onClick="fntViewRoller(<?=$persiana['ID_MOTORIZADA']?>)" title="Ver Roller"><i class="far fa-eye"></i> </button> -->
                          <button class="btn btn-primary btn-sm" onClick="fntEditRoller(<?=$persiana['ID_MOTORIZADA']?>)" title="Editar Roller"><i class="fas fa-pencil-alt"></i></button>
                          <button class="btn btn-danger btn-sm"   onClick="fntDelRoller(<?=$persiana['ID_MOTORIZADA']?>)" title="Eliminar Roller"><i class="far fa-trash-alt"></i></button>                                 
                    </td>
                  </tr>
                  <?php 
                            }
          ...
          <form id="formPrice" name="formPrice" class="form-horizontal">
            ...
            <div class="row d-print-none mt-2">
             ...
             <br><br>
             <?php
                    if(isset($roller['mo_nota'])){ ?>
                    <p>No hay notas especiales</p>
                    <?php }else{ ?>
                      <a class="btn btn-outline-info" href="<?=base_url().'/motorizacion/PDFNOTES/'.$orden['idpedido']?>" target="_blanck"><i class="fa fa-print"></i> Imprimir notas especiales</a> 
                    <?php } ?>
              <a class="btn btn-success" href="<?=base_url().'/Motorizacion/facturacliente/'.$orden['idpedido']?>" target="_blanck"><i class="fa fa-print"></i> PDF Cliente</a> 
               <a class="btn btn-warning" href="<?=base_url().'/motorizacion/facturaproduccion/'.$orden['idpedido']?>" target="_blanck" ><i class="fa fa-print"></i> PDF Produccion</a>
              </div> 
          </div>
          </form>
        </section>
