        ...
        <section id="sPedido" class="invoice">
          ...
          <div>
            ...
            <?php if($_SESSION['permisosMod']['w']){ ?>
                <button class="btn btn-primary" type="button" onclick="openModalRomana();"><i class="fas fa-plus-circle"></i> Nuevo</button>
              <?php } ?>
            </h2>
        </div>
        <br>
          ...
          <div class="row">
            ...
            <table class="table table-hover table-bordered" id="tableBlindsRomana">
                      <thead>
                        <tr>
                          <th>Cant</th>
                          <th>Tipo</th>
                    ...
                    $importe = 0;
                        if(count($roller) > 0){
                            foreach ($roller as $persiana) {
                                $importe +=$persiana['ro_precio'];
                     ?>
                  <tr>
                    <td><?= $persiana['ro_cantidad'] ?></td>
                    <!-- <td><= $persiana['en_identificacion'] ?></td> -->
                    <?php
                    if(empty($persiana['ro_nota'])){ ?>
                    <td><span class="badge badge-success">Normal</span></td>
                    <?php }else{ ?>
                      <td><span class="badge badge-warning">Especial</span></td>
                    <?php }?>

                    <td><?= $persiana['ro_tiporo'] ?></td>
                    <td><?= $persiana['ro_ancho'] ?></td>
                    <td><?= $persiana['ro_largo'] ?></td>
                    <td><?= $persiana['ro_color_tela'] ?></td>
                    <td><?= $persiana['ro_precio'] ?></td>
                    <td>
                          <!-- <button class="btn btn-info btn-sm"    onClick="fntViewRoller(<?=$persiana['ID_ROMANA']?>)" title="Ver Persiana"><i class="far fa-eye"></i> </button> -->
                          <button class="btn btn-primary btn-sm" onClick="fntEditRoller(<?=$persiana['ID_ROMANA']?>)" title="Editar Persiana"><i class="fas fa-pencil-alt"></i></button>
                          <button class="btn btn-danger btn-sm"   onClick="fntDelRoller(<?=$persiana['ID_ROMANA']?>)" title="Eliminar Persiana"><i class="far fa-trash-alt"></i></button>                                 
                    </td>
                  </tr>
                  <?php 
                            }
          ...
          <form id="formPrice" name="formPrice" class="form-horizontal">
            ...
            <div class="row d-print-none mt-2">
             ...
             <br><br>
             <?php
                    if(isset($roller['ro_nota'])){ ?>
                    <p>No hay notas especiales</p>
                    <?php }else{ ?>
                      <a class="btn btn-outline-info" href="<?=base_url().'/romana/PDFNOTES/'.$orden['idpedido']?>" target="_blanck"><i class="fa fa-print"></i> Imprimir notas especiales</a> 
                    <?php } ?>
             ...
             <php }
              ?>         -->
              <a class="btn btn-success" href="<?=base_url().'/romana/facturacliente/'.$orden['idpedido']?>" target="_blanck"><i class="fa fa-print"></i> PDF Cliente</a> 
               <a class="btn btn-warning" href="<?=base_url().'/romana/facturaproduccion/'.$orden['idpedido']?>" target="_blanck" ><i class="fa fa-print"></i> PDF Produccion</a>
              </div> 
          </div>
          </form>
        </section>
