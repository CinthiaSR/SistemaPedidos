        ...
        <section id="sPedido" class="invoice">
          ...
          <div>
            ...
            <?php if($_SESSION['permisosMod']['w']){ ?>
                <button class="btn btn-primary" type="button" onclick="openModalArcos();"><i class="fas fa-plus-circle"></i> Nuevo</button>
              <?php } ?>
            </h2>
        </div>
        <br>
          ...
          <div class="row">
            ...
            <table class="table table-hover table-bordered" id="tableArcos">
                      <thead>
                        <tr>
                          <th>Cant.</th>
                          <th>Ubicacion</th>
                    ...
                    $importe = 0;
                        if(count($arcos) > 0){
                            foreach ($arcos as $persiana) {
                                $importe +=$persiana['arcos_preciototal'];
                     ?>
                  <tr>
                    <!-- <td>?= $persiana['en_cantidad'] ?></td> -->
                    <td><?= $persiana['cantidad_arcos'] ?></td>
                    <td><?= $persiana['arcos_identificacion'] ?></td>
                    <?php
                    if(empty($persiana['arcos_nota'])){ ?>
                    <td><span class="badge badge-success">Normal</span></td>
                    <?php }else{ ?>
                      <td><span class="badge badge-warning">Especial</span></td>
                    <?php }?>
                    <!-- <td><= $persiana['shut_nota'] ?></td> -->
                    <td><?= $persiana['arcos_base'] ?></td>
                    <td><?= $persiana['arcos_altura'] ?></td>
                    <td><?= $persiana['arcos_color'] ?></td>
                    <td>$ <?= $persiana['arcos_preciototal'] ?></td>

                    <td>
                          <button class="btn btn-primary btn-sm" onClick="fntEditShutter(<?=$persiana['ID_ARCOS']?>)" title="Editar Shutter"><i class="fas fa-pencil-alt"></i></button>
                          <button class="btn btn-danger btn-sm"   onClick="fntDelShutter(<?=$persiana['ID_ARCOS']?>)" title="Eliminar Shutter"><i class="far fa-trash-alt"></i></button>                                 
                    </td>
                  </tr>
                  <?php 
                            }
               ...
               <!-- <php 
                 if(empty($persiana['arcos_nota'])){ ?>
                   <p>No hay notas especiales</p>
                    <php }else{ ?>
                   <a class="btn btn-outline-info" href="<=base_url().'/arcos/PDFNOTES/'.$orden['idpedido']?>" target="_blanck"><i class="fa fa-print"></i> Imprimir notas especiales</a> 
                 <php }?> -->
            <a class="btn btn-outline-info" href="<?=base_url().'/arcos/PDFNOTES/'.$orden['idpedido']?>" target="_blanck"><i class="fa fa-print"></i> Imprimir notas especiales</a> 
              <a class="btn btn-success" href="<?=base_url().'/arcos/facturacliente/'.$orden['idpedido']?>" target="_blanck"><i class="fa fa-print"></i> PDF Cliente</a> 
               <a class="btn btn-warning" href="<?=base_url().'/arcos/facturaproduccion/'.$orden['idpedido']?>" target="_blanck" ><i class="fa fa-print"></i> PDF Produccion</a>
              </div> 
          </div>
          </form>
          <!-- <div class="row d-print-none mt-2">
